...
?>
<h1 class="text-primary"><i class="fas fa-user-plus"></i>  Editar Información del Estudiante<small class="text-warning"> Editar</small></h1>
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
	...
	<?php
		if (isset($id)) {
			$query = "SELECT `id`, `name`, `roll`, `class`, `city`, `pcontact`, `photo`, `datetime`,`state`  FROM `student_info` WHERE `id`=$id";
...
<div class="col-sm-6">
	<form enctype="multipart/form-data" method="POST" action="">
		<div>
			<div id="estado">
				<?php
				echo ($row['state']==1?"Activo":"Inactivo"); 
				?>
			</div>
			<a class="btn btn-success" onClick="verestado()"> Cambiar estado</a>
		...
		<div class="form-group">
	  	...
	  	<div class="form-group">
		    ...
		    <input name="pcontact" type="text" class="form-control" id="pcontact" value="<?php echo $row['pcontact']; ?>" pattern="[0-9]{9}" placeholder="+51..." required="">
	  	</div>
	  	<div class="form-group">
...
<script type="text/javascript">
    	function verestado() {
			...
			$.ajax({ 
				url:"state.php",
				type:"POST",
				data:{id:ida,state:st}				
			}).done(function(result){
				$("#estado").html((st==1?"Activo":"Inactivo"));
				$("#state").val(st==1?0:1);
			});
		}
